  ...
  return (
    <div className="space-y-10 max-w-2xl">
      ...
      <section className="space-y-4">
        ...
        {apiStats.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-4">
            No API calls logged yet — run the <code className="bg-gray-100 px-1 rounded text-xs">api-logs-migration.sql</code> in Supabase first.
          </p>
        ) : (
          <>
            {/* Summary stats table */}
                  ...
                  {apiStats.map(stat => (
                    <tr key={stat.api_name} className="bg-white hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className={`font-medium ${stat.success_pct >= 90 ? 'text-green-600' : stat.success_pct >= 50 ? 'text-yellow-600' : 'text-red-600'}`}>
                          {stat.success_pct}%
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right text-gray-400 text-xs">
                        {stat.last_called ? new Date(stat.last_called).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : '—'}
                      </td>
                    </tr>
                  ))}
                </tbody>
                        ...
                        {logs.map(log => (
                          <tr key={log.id} className={`${log.success ? 'bg-white' : 'bg-red-50'}`}>
                            <td className="px-4 py-2 text-gray-500 font-mono whitespace-nowrap">
                              {new Date(log.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                            </td>
